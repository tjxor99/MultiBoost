...
//----------------------------------------------------------------
    ...
    void Wish::initialize( vector< AlphaReal >& vals )
    {
...
//----------------------------------------------------------------
    ...
    void Wish::receiveReward( int armNum, AlphaReal reward )
    {
        // cout << "Wish!"; // Verified that this is being called.
        _T[ armNum ]++;
        ...
        {
          if (i == armNum){
            _r_av[i] = ((_T[i] - 1)*_r_av[i] + reward) / _T[i];
          }
          _w[i] = _eta*_time*_r_av[i];
          updateithValue( i );
        }
