...
//----------------------------------------------------------------
    ...
    ABE::ABE( void ) : Exp3G()
    {
        ...
        _Cparam = 1;
        _alpha = 0.1; // The exponent of outer time for which exploration occurs (i^\alpha)
    }
...
// Check End
        ...
        if ( _inner_time <= (int)_numOfArms * pow(_Cparam, _alpha) ) { // Using Cparam
            int next_arm;
            ...
            // if (0) // No Explore
            if (_inner_time == (int)(_numOfArms * pow(_Cparam, _alpha) + _numOfArms * pow((double)2.0, _out_time)))
            {
                fill( _w.begin(), _w.end(), -10.0 );
                _w[0] = 1;
                _inner_time = 0;
